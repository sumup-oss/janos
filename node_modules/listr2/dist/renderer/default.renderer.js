...
class DefaultRenderer {
                ...
                if (task.hasTitle()) {
                    if (!(tasks.some((task) => task.hasFailed()) && !task.hasFailed() && task.options.exitOnError !== false && !(task.isCompleted() || task.isSkipped()))) {
                        ...
                        if (task.hasFailed() && this.getSelfOrParentOption(task, 'collapseErrors')) {
                            ...
                            output = [
                                ...output,
                                this.formatString((task.message.skip && this.getSelfOrParentOption(task, 'showSkipMessage') ? task.message.skip : task.title) +
                                    (this.getSelfOrParentOption(task, 'suffixSkips') ? chalk_1.default.dim(' [SKIPPED]') : ''), this.getSymbol(task), level)
                            ];
                        }
    ...
    getSymbol(task, data = false) {
        ...
        if (task.isRollingBack() && !data) {
            return ((_b = this.options) === null || _b === void 0 ? void 0 : _b.lazy) ? chalk_1.default.red(figures_1.default.main.warning) : chalk_1.default.red(this.spinner[this.spinnerPosition]);
        }
